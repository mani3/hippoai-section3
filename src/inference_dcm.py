...
def get_predicted_volumes(pred):
    ...
    total_volume = np.where(pred != 0, 1, 0).sum()
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    for i, n in enumerate(slice_nums) :
        s = orig_vol[n]
        p = pred_vol[n]
        w, h = s.shape[1] * 4, s.shape[0] * 4
        
        s = med_reshape(np.expand_dims(s, 0), (1, 64, 64)).reshape((64, 64))
        nd_img = np.flip((s / np.max(s)) * 0xff).T.astype(np.uint8)
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((w, h))
        
        pred_img = np.flip(p * 127).T.astype(np.uint8)
        im = np.zeros((pred_img.shape[0], pred_img.shape[1], 3)).astype(np.uint8)
        im[:, :, 0] = pred_img
        pred_img = Image.fromarray(im, mode='RGB').convert("RGBA").resize(s.shape).resize((w, h))
        pred_img.putalpha(128)
        
        pil_i = Image.alpha_composite(pil_i, pred_img)
        pimg.paste(pil_i, box=(160 + 200 * i, 320))
...
def get_series_for_inference(path):
    ...
    series_for_inference = [pydicom.dcmread(f) for f in paths]
    series_for_inference = [s for s in series_for_inference if s.SeriesDescription == 'HippoCrop']
